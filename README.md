...
<h2>Installation Steps</h2>
   ...
   >**Note**: Windows 10 machine with 2 or 4 cpu's will make the overall connection stronger to avoid troubles involving slow connection.
Allow the VM to create a new Vnet which should be done by default.

We can do a quick search for `Virtual Machine` and as we create the virtual machine, we will have the option to create the Resource Group. Here we select `Create New` to name the Resource Group 'RG-osTicket' and build out the Virtual Machine (VM) with the provided settings pictured (below). 

<p align="center"><img src="https://i.imgur.com/jz0e59n.png" height="70%" width="70%" alt="image of vm settings"/> </p>

   >**Note**: Be sure to check the box at the bottom of the screen stating 'I confirm I have an eligible Windows 10/11 license with multi-tenant hosting rights.' or else you will receive an error message when you choose to 'Review + Create'.
...
"Small steps lead to big victories." - Anonymous
